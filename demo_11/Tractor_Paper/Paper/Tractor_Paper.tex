\documentclass[11pt]{book}
\usepackage{fullpage}
\usepackage{palatino}
\usepackage{amsfonts,amsmath,amssymb}

% Packages for displaying code:
\usepackage{listings}
\usepackage{textcomp}
\usepackage{color}

% Color settings used in the code below:
\definecolor{dkgreen}{rgb}{0,0.6,0}
\definecolor{gray}{rgb}{0.5,0.5,0.5}
\definecolor{mauve}{rgb}{0.58,0,0.82}

% Settings for the formatting of the code on display:
\lstset{frame=tb,
  language=R,
  aboveskip=3mm,
  belowskip=3mm,
  showstringspaces=false,
  columns=flexible,
  basicstyle={\small\ttfamily},
  numbers=none,
  numberstyle=\tiny\color{gray},
  keywordstyle=\color{blue},
  commentstyle=\color{dkgreen},
  stringstyle=\color{mauve},
  breaklines=true,
  breakatwhitespace=true,
  tabsize=3
}

% Packages for Figures:
% Without conversion of eps to pdf:
% \usepackage{graphicx}
% With conversion of eps to pdf,
% which depends on platform.
\ifx\pdftexversion\undefined
    \usepackage[dvips]{graphicx}
\else
    \usepackage[pdftex]{graphicx}
    \usepackage{epstopdf}
    \epstopdfsetup{suffix=}
\fi

\usepackage{subfig}

% Package for displaying inline verbatim commands in footnotes.
\usepackage{fancyvrb}

% This allows pdflatex to print the curly quotes in the
% significance codes in the output of the GAM.
\UseRawInputEncoding

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{document}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\input{Tractor_Title_Page}

\pagebreak
\chapter{Introduction}
\input{Tractor_Introduction}

\pagebreak
\chapter{Data Description}
\input{Tractor_Data_Desc}

\pagebreak
\chapter{Analysis of the Dependent Variable}
\input{Tractor_Price_Density}

\pagebreak
\chapter{Transforming the Dependent Variable}
\input{Tractor_Box_Cox}

% \pagebreak
% \chapter{Preliminary Tabular Analysis}
% \input{Tractor_Tables}

\pagebreak
\chapter{Preliminary Graphical Analysis}
\input{Tractor_Data_Vis}

\pagebreak
\chapter{Regression Modelling with Hedonic Price Theory}
\input{Tractor_Reg_Models}

\pagebreak
\chapter{Nonparametric Regression Models}
\input{Tractor_Nonparametric}

\pagebreak
\chapter{Transforming the Explanatory Variables}
\input{Tractor_Box_Tidwell}

\pagebreak
\chapter{Sample Selection Models}
\input{Tractor_Sample_Selection}

% \pagebreak
% \chapter{References}
% \input{Tractor_References}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\end{document}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

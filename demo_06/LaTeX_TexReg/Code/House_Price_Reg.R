...
# wd_path <- '~/GitHub/QMB6912S22/demo_07/LaTeX_TexReg'
# setwd(wd_path)
...
tab_file_name <- 'lm_full_model.tex'
...
texreg(lm_full_model,
       ...
       # sideways = TRUE,
       # use.packages = FALSE,
       custom.model.names = 'Name of My Model',
...
# You can also use the pdf functions to save the figure to a file in one step.
...
# Now use either the pdf or the setEPS and postscript functions
...
##################################################
# Showing several regression models in one table
##################################################

# In the texreg function, the agrument for the
# lm model object can take a list of lm model objects.
# This is useful for comparing several models at once.
# To illustrate the method, I will estimate a series of models.



#--------------------------------------------------
# Estimating Additional Regression Models
# Models 2-3: Removing Variables
#--------------------------------------------------


# Estimate another regression model.
lm_model_2 <- lm(data = housing_data,
...
# Estimate a final regression model.
lm_model_3 <- lm(data = housing_data,
                 formula = house_price ~ income)
...
# Print the output to a LaTeX file.
tab_file_name <- 'lm_all_models.tex'
out_file_name <- sprintf('%s/%s', tab_dir, tab_file_name)
texreg(l = list(lm_full_model,
                lm_model_2,
                lm_model_3),
       custom.model.names = c('Full Model', 'Income Only', 'No Earthquakes'),
       digits = 3,
       file = out_file_name,
